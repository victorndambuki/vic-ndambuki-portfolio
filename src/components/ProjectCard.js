...
const TAG_STYLES = { laser: 'tag tag-laser', '3d': 'tag tag-3d', design: 'tag tag-design' }
const TAG_LABELS = { laser: 'Laser Cutting', '3d': '3D Printing', design: 'Design' }
  ...
  return (
    <article
      ...
      style={{
        ...
        transition: `opacity 0.9s cubic-bezier(0.16,1,0.3,1) ${index * 60}ms, transform 0.9s cubic-bezier(0.16,1,0.3,1) ${index * 60}ms`,
      }}
    >
      <div className={`flex flex-col ${isEven ? 'lg:flex-row' : 'lg:flex-row-reverse'}`}>
        ...
        <div className="relative w-full lg:w-1/2 h-72 sm:h-96 lg:h-auto lg:min-h-[480px]">
          ...
          <div className="absolute top-4 left-4 z-20 flex border border-ash/10 overflow-hidden rounded-sm">
            {['model', ...(imagePath ? ['image'] : [])].map(tab => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-3 py-1.5 font-mono text-xs tracking-widest uppercase transition-colors duration-200 ${
                  activeTab === tab ? 'bg-copper text-ink' : 'bg-ink-800/80 text-ash-400 hover:text-ash'
                }`}
              >
                {tab === 'model' ? '3D' : 'Photo'}
              </button>
          ...
          {/* 3D viewer */}
          <div className={`absolute inset-0 transition-opacity duration-400 ${activeTab === 'model' ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
            <ModelViewer modelPath={modelPath} />
          </div>

          {/* Photo */}
          {imagePath && (
            <div className={`absolute inset-0 transition-opacity duration-400 ${activeTab === 'image' ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          ...
          {!modelPath && activeTab === 'model' && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-ink-800">
              <span className="font-display text-5xl text-ash/10">
                {project.tags[0] === 'laser' ? '◈' : project.tags[0] === '3d' ? '⬡' : '✏'}
              </span>
              <span className="font-mono text-xs text-ash/20 tracking-widest">ADD MODEL FILE</span>
            </div>
          )}
        </div>
        ...
        <div className="w-full lg:w-1/2 p-8 sm:p-10 lg:p-14 flex flex-col justify-center border-t lg:border-t-0 border-ash/5"
          style={{ borderLeft: isEven && typeof window !== 'undefined' && window.innerWidth >= 1024 ? '1px solid rgba(240,235,227,0.05)' : undefined }}
        >
          {/* Tags */}
          <div className="flex flex-wrap gap-2 mb-6">
            {project.tags.map(t => (
              <span key={t} className={TAG_STYLES[t] || 'tag tag-design'}>{TAG_LABELS[t] || t}</span>
            ))}
          </div>
